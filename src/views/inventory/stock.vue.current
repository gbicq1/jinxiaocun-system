      ...
      <div class="stock-detail-dialog" @click.stop>
        ...
        <div v-if="selectedRow" class="stock-detail-body">
          ...
          <div class="stock-detail-table-wrapper">
            <el-table :data="ledgerEntries" style="width: 100%" :show-summary="false">
              <el-table-column prop="productCode" label="产品编码" width="120" fixed />
              ...
              <el-table-column prop="runningQty" label="库存数量" width="120" />
...
const calculateStockFromTransactions = (productList: any[]) => {
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    // 只处理出入库相关的键
    const isInbound = inboundKeys.includes(key) || key.toLowerCase().includes('inbound')
    const isOutbound = outboundKeys.includes(key) || (key.toLowerCase().includes('outbound') && !key.toLowerCase().includes('return'))
