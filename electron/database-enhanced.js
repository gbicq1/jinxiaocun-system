...
exports.EnhancedInventoryDatabase = void 0;
const better_sqlite3_1 = __importDefault(require("better-sqlite3"));
class EnhancedInventoryDatabase {
    constructor(dbPath) {
        this.db = null;
     ...
     * 这是解决成本价不一致问题的核心
     */
    getOpeningBalance(productId, warehouseId, periodStart) {
      ...
      LIMIT 1
    `).get(productId, warehouseId, previousDayStr);
        if (snapshot) {
     ...
     * 从出入库流水表计算库存
     */
    calculateStockFromTransactions(productId, warehouseId, beforeDate) {
    ...
    `).all(productId, warehouseId, beforeDate);
        let quantity = 0;
        let totalCost = 0;
    ...
    `);
        const result = stmt.run(data.productId, data.productCode, data.warehouseId, data.snapshotDate, data.snapshotType, data.quantity, data.unitCost, data.totalCost, data.monthlyInboundQty || 0, data.monthlyInboundCost || 0, data.monthlyOutboundQty || 0, data.monthlyOutboundCost || 0, data.source || 'calculated');
    ...
    `);
        const result = stmt.run(data.settlementNo, data.productId, data.productCode, data.warehouseId, data.periodStart, data.periodEnd, data.openingQty, data.openingCost, data.openingUnitCost, data.inboundQty, data.inboundCost, data.outboundQty, data.outboundCost, data.weightedAvgUnitCost, data.closingQty, data.closingCost, data.closingUnitCost, data.status || 'draft', data.remark || '');
        return result.lastInsertRowid;
    }
    /**
     ...
     * 这是过渡方案，确保数据不丢失
     */
    syncLocalStorageToDatabase(localStorage) {
        let syncCount = 0;
        try {
            // 同步出入库记录
            const keys = [
                ...
                try {
                    ...
                    records.forEach((rec) => {
                        ...
                        items.forEach((item) => {
                            ...
                            else if (key.includes('outbound'))
                                transactionType = 'outbound';
                            else if (key.includes('transfer')) {
                                if (rec.fromWarehouseId === warehouseId)
                                    transactionType = 'transfer_out';
                                else if (rec.toWarehouseId === warehouseId)
                            ...
                            try {
                                insertStmt.run(rec.voucherNo || rec.transferNo || '', productId, productCode, warehouseId, date, time, time, transactionType, key, rec.voucherNo || rec.transferNo || '', quantity, unitCost, totalCost, key, rec.id || '');
                                syncCount++;
                            }
                            catch (e) {
                                // 忽略重复数据
...
exports.EnhancedInventoryDatabase = EnhancedInventoryDatabase;
exports.default = EnhancedInventoryDatabase;
