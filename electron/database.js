...
class InventoryDatabase {
    ...
    migrateDatabase() {
        try {
            ...
            const prColumns = this.db.prepare("PRAGMA table_info(purchase_return_items)").all();
            ...
            const srMainColumns = this.db.prepare("PRAGMA table_info(sales_returns)").all();
            ...
            const sriColumns = this.db.prepare("PRAGMA table_info(sales_return_items)").all();
            const sriNewCols = [
            ...
            const prMainColumns = this.db.prepare("PRAGMA table_info(purchase_returns)").all();
            ...
            const priColumns = this.db.prepare("PRAGMA table_info(purchase_return_items)").all();
            const priNewCols = [
                { name: 'product_name', sql: 'ALTER TABLE purchase_return_items ADD COLUMN product_name VARCHAR(200)' },
                { name: 'specification', sql: 'ALTER TABLE purchase_return_items ADD COLUMN specification VARCHAR(200)' },
                { name: 'unit', sql: 'ALTER TABLE purchase_return_items ADD COLUMN unit VARCHAR(50)' },
    ...
    `);
        ...
        try {
            this.db.exec(`ALTER TABLE purchase_inbound ADD COLUMN invoice_date DATETIME`);
        }
        catch (_) { }
        try {
            this.db.exec(`ALTER TABLE purchase_inbound ADD COLUMN invoiced_amount DECIMAL(10,2) DEFAULT 0`);
        }
        catch (_) { }
    ...
    getInboundList(page = 1, pageSize = 10, where, params) {
        ...
        const enrichedData = data.map((record) => {
            ...
            // 加载供应商和仓库信息
            const supplier = this.db.prepare('SELECT name FROM suppliers WHERE id = ?').get(record.supplier_id);
            const warehouse = this.db.prepare('SELECT name FROM warehouses WHERE id = ?').get(record.warehouse_id);
            return {
                id: record.id,
    ...
    updateInbound(inbound) {
        try {
            ...
            const transaction = this.db.transaction(() => {
                ...
                // 只映射需要更新的字段
                if (inbound.inbound_no !== undefined)
                    mainData.inbound_no = inbound.inbound_no;
                if (inbound.inbound_date !== undefined)
                    mainData.inbound_date = inbound.inbound_date;
                if (inbound.supplier_id !== undefined)
                    mainData.supplier_id = inbound.supplier_id;
                if (inbound.warehouse_id !== undefined)
                    mainData.warehouse_id = inbound.warehouse_id;
                if (inbound.total_amount !== undefined)
                    mainData.total_amount = inbound.total_amount;
                if (inbound.paid_amount !== undefined)
                    mainData.paid_amount = inbound.paid_amount;
                if (inbound.invoice_type !== undefined)
                    mainData.invoice_type = inbound.invoice_type;
                if (inbound.invoice_issued !== undefined)
                    mainData.invoice_issued = inbound.invoice_issued;
                if (inbound.invoice_date !== undefined)
                    mainData.invoice_date = inbound.invoice_date;
                if (inbound.invoiced_amount !== undefined)
                    mainData.invoiced_amount = inbound.invoiced_amount;
                if (inbound.status !== undefined)
                    mainData.status = inbound.status;
                if (inbound.remark !== undefined)
                    mainData.remark = inbound.remark;
    ...
    getOutboundList(page = 1, pageSize = 10, where, params) {
        ...
        const enrichedData = data.map((record) => {
            ...
            const items = this.db.prepare(itemsSql).all(record.id);
            // 加载客户和仓库信息
            const customer = this.db.prepare('SELECT name FROM customers WHERE id = ?').get(record.customer_id);
            const warehouse = this.db.prepare('SELECT name FROM warehouses WHERE id = ?').get(record.warehouse_id);
            return {
                id: record.id,
    ...
    addPurchaseReturn(returnData) {
        try {
            ...
            const transaction = this.db.transaction(() => {
                ...
                const items = (returnData.items || []).filter((item) => item.quantity > 0);
    ...
    getPurchaseReturns(page = 1, pageSize = 10) {
        const offset = (page - 1) * pageSize;
            ...
            return {
                ...
                items: items.map((item) => {
                    ...
                    if ((!unitPrice || unitPrice === 0 || !totalAmount || totalAmount === 0) && item.product_id) {
                        try {
                            // 获取退货单主表信息
                            const returnMain = this.db.prepare('SELECT original_inbound_no FROM purchase_returns WHERE id = ?').get(item.return_id);
                            console.log('[数据库] 退货单主表信息:', returnMain);
                            if (returnMain && returnMain.original_inbound_no) {
                                // 从原入库单中获取对应商品的价格
                                const inbound = this.db.prepare('SELECT id FROM purchase_inbound WHERE inbound_no = ?').get(returnMain.original_inbound_no);
                                console.log('[数据库] 原入库单 ID:', inbound);
                                if (inbound) {
                                    const inboundItem = this.db.prepare('SELECT unit_price, unit_price_ex, tax_rate, tax_amount, total_amount FROM purchase_inbound_items WHERE inbound_id = ? AND product_id = ?').get(inbound.id, item.product_id);
    ...
    updatePurchaseReturn(returnData) {
        try {
            console.log('【Electron 后端】更新采购退货单数据:', returnData);
            // 开启事务
            const transaction = this.db.transaction(() => {
                const id = returnData.id;
                const items = (returnData.items || []).filter((item) => item.quantity > 0); // 只保存数量>0的商品
                // 删除原有明细
    ...
    getSalesReturns(page = 1, pageSize = 10) {
        const offset = (page - 1) * pageSize;
    ...
    // ==================== 系统管理 ====================
    getRoles() {
        return this.db.prepare('SELECT * FROM roles ORDER BY id').all();
    }
    addRole(data) {
    ...
    // ==================== 库存查询 ====================
    getAllStocks(endDate) {
        const df = (dateCol) => endDate ? ` AND ${dateCol} <= '${endDate}'` : '';
        const products = this.db.prepare('SELECT id, code, name, category, unit, spec, cost_price, warning_quantity FROM products WHERE status = 1').all();
        const warehouses = this.db.prepare('SELECT id, code, name FROM warehouses WHERE status = 1').all();
        const result = [];
    ...
    calculateStockForProductAndWarehouse(productId, warehouseId, endDate) {
        let totalStock = 0;
        const df = (dateCol) => endDate ? ` AND ${dateCol} <= '${endDate}'` : '';
        try {
      ...
      `).get(productId, warehouseId) || { total_qty: 0 };
    ...
    getProductLedger(productId, warehouseId, startDate, endDate) {
        const items = [];
